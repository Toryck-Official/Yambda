...
def update_memory_from_simulated_step(
    ...
    for row_idx in torch.nonzero(active_mask.detach().cpu(), as_tuple=False).view(-1).tolist():
        ...
        if failure_type == 2 or event_id == EVENT_TYPE_TO_ID["dislike"]:
            regret_type_id, strength = 2, 1.2
...
def select_actions(
    ...
    item_features: np.ndarray | torch.Tensor | None = None,
    future_predictor: FuturePredictor | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    feed = actor_obs(obs)
    pool_tokens = None
    pool_phis = None
    ...
    if bool(args.use_future_predictor) and float(args.predictor_beta) > 0.0:
        ...
        return decode_with_future_predictor(
            future_predictor,
            item_features,
            obs,
            decoder,
...
def main() -> None:
    ...
    item_features = maybe_cache_item_features_on_gpu(item_features_np, device, use_future_predictor and args.cache_item_features_on_gpu)
    ...
    for batch in loader:
        ...
        if seen + batch_n > int(args.num_episodes):
            ...
            batch_n = keep
        rollout_one_policy_batch(
            actor, base_env, batch, decoder, args, dense_item2sid, False, base_stats, pbar, base_rng,
            item_features, future_predictor,
        )
        rollout_one_policy_batch(
            actor, rapi_env, batch, decoder, args, dense_item2sid, True, rapi_stats, pbar, rapi_rng,
            item_features, future_predictor,
        )
        seen += batch_n
        pbar.set_postfix(
