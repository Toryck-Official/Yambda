...
PREDICTOR_MODEL_ROOT = PROJECT_ROOT.parent / "02_model"
if str(PREDICTOR_MODEL_ROOT) not in sys.path:
    sys.path.insert(0, str(PREDICTOR_MODEL_ROOT))
...
from regret_core.model.sid_policy import MultiLevelValueCritic, RegretAwareSIDPolicy, SIDActionQCritic  # noqa: E402
from predictor import FuturePredictor  # noqa: E402
from soft_state import SoftStateBuilder  # noqa: E402
from value import ValueHead  # noqa: E402
...
class SIDDecoder:
    ...
    def build_candidates(
    ...
    ) -> list[list[tuple[int, np.ndarray, float, float, float]]]:
        ...
        batch_candidates: list[list[tuple[int, np.ndarray, float, float, float]]] = []
        for row_idx in range(int(log_probs[0].shape[0])):
            candidates: list[tuple[int, np.ndarray, float, float, float]] = []
            if max_candidates is None:
                combo_iter = itertools.product(*[range(k) for _ in range(self.sid_levels)])
            else:
                beam_width = max(int(max_candidates) * 4, k)
                    ...
                    for prefix, prefix_score in beams:
                        for idx in range(k):
                            expanded.append(
                                (
                                    (*prefix, idx),
                                    prefix_score + float(top_values[level][row_idx, idx]),
                                )
                            )
                    expanded.sort(key=lambda item: item[1], reverse=True)
                    beams = expanded[:beam_width]
                combo_iter = (prefix for prefix, _score in beams)
            for combo in combo_iter:
                ...
                penalty = self._candidate_penalty(sid_tuple, row_idx, pool_tokens_np, pool_phis_np, layer_weights_np)
                adjusted_score = score - eta * penalty
                candidates.append((dense_item_id, sid_tuple, score, penalty, adjusted_score))
            if not candidates:
                fallback_item = int(fallback_np[row_idx])
                fallback_sid = self.sid_for_item(fallback_item)
                candidates.append((fallback_item, fallback_sid, 0.0, 0.0, 0.0))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--critic_type", default="action_q", choices=["action_q", "mlc"])
    parser.add_argument("--actor_weight", type=float, default=1.0)
    parser.add_argument(
        "--offline_aux_weight",
        type=float,
        default=0.0,
        help="Weight for HSRL-style reward-gated offline action supervision on logged transitions.",
    )
    parser.add_argument("--offline_aux_reward_threshold", type=float, default=0.0)
    parser.add_argument("--offline_aux_temperature", type=float, default=0.5)
    parser.add_argument("--offline_aux_bc_weight", type=float, default=1.0)
    parser.add_argument("--offline_aux_avoid_weight", type=float, default=1.0)
    parser.add_argument("--offline_aux_entropy_weight", type=float, default=0.0)
    ...
    parser.add_argument("--use_future_predictor", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--predictor_checkpoint", default="")
    parser.add_argument("--predictor_beta", type=float, default=0.1)
    parser.add_argument("--predictor_regret_eta", type=float, default=1.0)
    parser.add_argument("--predictor_candidate_k", type=int, default=32)
    parser.add_argument("--cache_item_features_on_gpu", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument(
        "--predictor_score_mode",
        default="future_return_minus_regret",
        choices=["future_return_minus_regret", "future_return", "predicted_reward_minus_regret", "predicted_reward", "regret_avoid"],
    )
    parser.add_argument("--predictor_score_norm", default="zscore", choices=["zscore", "none"])
    parser.add_argument("--use_model_based_predictor", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--future_value_checkpoint", default="")
    parser.add_argument("--model_predictor_checkpoint", default="")
    parser.add_argument("--model_q_weight", type=float, default=0.05)
    parser.add_argument("--model_q_gamma", type=float, default=-1.0)
    parser.add_argument("--model_q_clip", type=float, default=0.0)
    parser.add_argument("--model_q_mode", default="critic_target", choices=["critic_target"])
...
def load_future_predictor(path: str, item_dim: int, device: torch.device) -> FuturePredictor:
    if not path:
        raise ValueError("--use_future_predictor requires --predictor_checkpoint")
    ...
    print(f"[predictor] loaded={path} missing={len(missing)} unexpected={len(unexpected)}")
    return model


def resolve_project_path(path: str, base_root: Path) -> str:
    raw = str(path)
    if not raw:
        return raw
    candidate = Path(raw)
    ...
    if candidate.exists():
        return str(candidate)
    rooted = base_root / raw
    if rooted.exists():
        return str(rooted)
    return raw


def load_model_based_bundle(
    ...
    if not value_checkpoint:
        raise ValueError("--use_model_based_predictor requires --future_value_checkpoint")
    value_path = resolve_project_path(value_checkpoint, PROJECT_ROOT.parent)
    value_ckpt = torch.load(value_path, map_location="cpu")
    predictor_path = predictor_checkpoint
    if not predictor_path:
        predictor_ckpts = value_ckpt.get("predictor_ckpts", [])
        if not predictor_ckpts:
            raise ValueError("future value checkpoint does not record predictor_ckpts; set --model_predictor_checkpoint")
        predictor_path = str(predictor_ckpts[0])
    predictor_path = resolve_project_path(predictor_path, PROJECT_ROOT.parent)
    predictor = load_future_predictor(predictor_path, item_dim, device)
    predictor_cfg = value_ckpt.get("predictor_config", {})
    d_model = int(predictor_cfg.get("d_model", 128))
    ckpt_item_dim = int(value_ckpt.get("item_dim", item_dim))
    soft_state = SoftStateBuilder(item_dim=ckpt_item_dim, d_model=d_model).to(device)
    ...
    soft_state.eval()
    value_head.eval()
    for module in (predictor, soft_state, value_head):
        for param in module.parameters():
            param.requires_grad_(False)
    print(
        "[model_predictor] loaded "
        f"value={value_path} predictor={predictor_path} d_model={d_model} item_dim={ckpt_item_dim}"
    )
    return {
        "predictor": predictor,
...
def maybe_cache_item_features_on_gpu(
    ...
    if device.type != "cuda":
        print("[features] cache_item_features_on_gpu requested but device is not cuda; using mmap features")
        return item_features
    tensor = torch.from_numpy(np.array(item_features, dtype=np.float32, copy=True)).to(device=device)
    gb = tensor.numel() * tensor.element_size() / (1024.0 ** 3)
    print(f"[features] cached dense item features on {device}: shape={tuple(tensor.shape)} size={gb:.2f}GB")
    return tensor


def lookup_item_features(item_features: np.ndarray | torch.Tensor, dense_ids: torch.Tensor, device: torch.device) -> torch.Tensor:
    if isinstance(item_features, torch.Tensor):
        ids = dense_ids.to(item_features.device).long().clamp(min=0, max=int(item_features.shape[0]) - 1)
        flat = ids.reshape(-1)
        return item_features.index_select(0, flat).reshape(*ids.shape, int(item_features.shape[1])).to(device)
    ids_np = dense_ids.detach().cpu().numpy().astype(np.int64)
    ids_np = np.clip(ids_np, 0, int(item_features.shape[0]) - 1)
    features = np.array(item_features[ids_np], dtype=np.float32, copy=True)
    return torch.from_numpy(features).to(device)


def predictor_score(out: dict[str, torch.Tensor], args: argparse.Namespace) -> torch.Tensor:
    mode = str(args.predictor_score_mode)
    regret_penalty = float(args.predictor_regret_eta) * out["future_regret_prob"]
    if mode == "future_return_minus_regret":
        return out["predicted_future_return"] - regret_penalty
    if mode == "future_return":
        return out["predicted_future_return"]
    if mode == "predicted_reward_minus_regret":
        return out["predicted_reward"] - regret_penalty
    if mode == "predicted_reward":
        return out["predicted_reward"]
    return -out["future_regret_prob"]


def normalize_candidate_score(score: torch.Tensor, mask: torch.Tensor, mode: str) -> torch.Tensor:
    if mode != "zscore":
        return score
    mask_f = mask.float()
    count = mask_f.sum(dim=1, keepdim=True).clamp_min(1.0)
    mean = (score * mask_f).sum(dim=1, keepdim=True) / count
    var = (((score - mean) * mask_f) ** 2).sum(dim=1, keepdim=True) / count
    return torch.where(mask, (score - mean) / torch.sqrt(var + 1e-6), torch.zeros_like(score))
...
def compute_model_q(
    ...
    action_features = lookup_item_features(item_features, action_items, device)
    pred = model_bundle["predictor"](actor_obs(obs), action_features)
    soft = model_bundle["soft_state"](pred["state_emb"], action_features, pred)
    next_value = model_bundle["value_head"](soft["next_state_emb"])
    gamma = float(args.model_q_gamma) if float(args.model_q_gamma) > 0.0 else float(args.gamma)
    model_q = pred["predicted_reward"] + gamma * next_value
    if float(args.model_q_clip) > 0.0:
        clip = float(args.model_q_clip)
        model_q = model_q.clamp(-clip, clip)
    return model_q.detach()


@torch.no_grad()
def decode_with_future_predictor(
    ...
    args: argparse.Namespace,
    rng: np.random.Generator,
    pool_tokens: torch.Tensor | None,
    pool_phis: torch.Tensor | None,
) -> tuple[torch.Tensor, torch.Tensor]:
    device = obs["history_features"].device
    candidates_by_row = decoder.build_candidates(
        logits_list,
        fallback_items=fallback_items,
        top_k=args.decode_top_k,
        max_candidates=int(max(1, args.predictor_candidate_k)),
        pool_tokens=pool_tokens,
        pool_phis=pool_phis,
        candidate_rerank=pool_tokens is not None and pool_phis is not None and bool(args.rapi_candidate_rerank),
    ...
    cap = int(max(1, args.predictor_candidate_k))
    max_len = max(1, min(cap, max(len(row) for row in candidates_by_row)))
    batch_size = len(candidates_by_row)
    cand_ids = np.zeros((batch_size, max_len), dtype=np.int64)
    cand_sids = np.zeros((batch_size, max_len, decoder.sid_levels), dtype=np.int64)
    policy_scores = torch.full((batch_size, max_len), -1.0e9, dtype=torch.float32, device=device)
    mask = torch.zeros((batch_size, max_len), dtype=torch.bool, device=device)
    for row_idx, row_candidates in enumerate(candidates_by_row):
        trimmed = row_candidates[:max_len]
        for col_idx, (item_id, sid_path, _score, _penalty, adjusted_score) in enumerate(trimmed):
            cand_ids[row_idx, col_idx] = int(item_id)
            cand_sids[row_idx, col_idx] = sid_path.astype(np.int64)
            policy_scores[row_idx, col_idx] = float(adjusted_score)
            mask[row_idx, col_idx] = True

    candidate_ids = torch.tensor(cand_ids, dtype=torch.long, device=device)
    candidate_features = lookup_item_features(item_features, candidate_ids, device)
    pred = predictor(actor_obs(obs), candidate_features)
    side_score = normalize_candidate_score(predictor_score(pred, args), mask, str(args.predictor_score_norm))
    final_scores = policy_scores + float(args.predictor_beta) * side_score
    final_scores = torch.where(mask, final_scores, torch.full_like(final_scores, -1.0e9))
    if str(args.action_mode) == "sample":
        probs = torch.softmax(final_scores / max(float(args.action_temperature), 1e-6), dim=1).cpu().numpy()
        choices = [int(rng.choice(max_len, p=probs[row_idx])) for row_idx in range(batch_size)]
    else:
        choices = final_scores.argmax(dim=1).detach().cpu().numpy().astype(np.int64).tolist()
    out_items = cand_ids[np.arange(batch_size), np.asarray(choices, dtype=np.int64)]
    out_sids = cand_sids[np.arange(batch_size), np.asarray(choices, dtype=np.int64)]
    return (
        torch.tensor(out_items, dtype=torch.long, device=device),
        torch.tensor(out_sids, dtype=torch.long, device=device),
    )
...
def critic_q(
    ...
    critic_out = critic({"context_list": out["context_list"], "target_sid": sid_paths})
    q = critic_out["q"]
    v_seq = critic_out.get("v_seq")
    if v_seq is None:
        v_seq = q.unsqueeze(1)
    return q, v_seq
...
def target_sid_from_dense_ids(
    ...
    ids_np = dense_ids.detach().cpu().numpy().astype(np.int64)
    valid_np = (ids_np > 0) & (ids_np < int(dense_item2sid.shape[0]))
    safe_ids = np.where(valid_np, ids_np, 0)
    sid_np = np.asarray(dense_item2sid[safe_ids], dtype=np.int64)
    valid_np = valid_np & np.all((sid_np >= 0) & (sid_np < int(sid_vocab_size)), axis=1)
    target_sid = torch.as_tensor(sid_np, dtype=torch.long, device=device)
    valid = torch.as_tensor(valid_np, dtype=torch.bool, device=device)
    return target_sid, valid


def compute_offline_aux_loss(
    ...
    if float(args.offline_aux_weight) <= 0.0:
        zero = torch.zeros((), dtype=torch.float32, device=device)
        return zero, {"offline_aux_loss": 0.0, "offline_aux_pos_loss": 0.0, "offline_aux_avoid_loss": 0.0, "offline_aux_token_acc": 0.0, "offline_aux_full_sid_acc": 0.0, "offline_aux_pos_share": 0.0, "offline_aux_examples": 0.0}

    target_sid, valid = target_sid_from_dense_ids(
        dense_item2sid,
        batch["target_dense_item_id"],
        device,
        sid_vocab_size,
    )
    ...
    feed = {
        ...
        "history_feedbacks": batch["history_feedbacks"].to(device).float(),
        "history_event_type_ids": batch["history_event_type_ids"].to(device).long(),
        "history_mask": batch["history_mask"].to(device).float(),
    }
    reward = batch["reward"].to(device).float()[valid]
    target_sid = target_sid[valid]
    out = actor(feed)
    sid_logits = [logits[valid] for logits in out["sid_logits"]]

    temp = max(float(args.offline_aux_temperature), 1e-6)
    threshold = float(args.offline_aux_reward_threshold)
    ...
    token_acc_sum = torch.zeros((), dtype=torch.float32, device=device)
    full_match = torch.ones(reward.shape[0], dtype=torch.bool, device=device)
    n_level = min(len(sid_logits), int(target_sid.shape[1]))
    for level in range(n_level):
        logits_l = sid_logits[level]
        z_l = target_sid[:, level].long().clamp(min=0, max=logits_l.shape[-1] - 1)
        log_probs_l = F.log_softmax(logits_l, dim=-1)
        ...
        correct_l = pred_l == z_l
        token_acc_sum = token_acc_sum + correct_l.float().mean()
        full_match = full_match & correct_l
        probs_l = log_probs_l.exp()
        entropy = entropy - (probs_l * log_probs_l).sum(dim=-1).mean()

    denom_level = max(n_level, 1)
    nll = nll / denom_level
    avoid = avoid / denom_level
    entropy = entropy / denom_level
    pos_loss = (pos_weight * nll).sum() / pos_weight.sum().clamp_min(1e-6)
    avoid_loss = (neg_weight * avoid).sum() / neg_weight.sum().clamp_min(1e-6)
    aux_loss = (
        float(args.offline_aux_bc_weight) * pos_loss
        + float(args.offline_aux_avoid_weight) * avoid_loss
        - float(args.offline_aux_entropy_weight) * entropy
    )
    token_acc = token_acc_sum / denom_level
    metrics = {
        "offline_aux_loss": float(aux_loss.detach().cpu()),
        "offline_aux_pos_loss": float(pos_loss.detach().cpu()),
        "offline_aux_avoid_loss": float(avoid_loss.detach().cpu()),
        "offline_aux_entropy": float(entropy.detach().cpu()),
        "offline_aux_token_acc": float(token_acc.detach().cpu()),
        "offline_aux_full_sid_acc": float(full_match.float().mean().detach().cpu()),
        "offline_aux_pos_share": float((reward >= threshold).float().mean().detach().cpu()),
        "offline_aux_examples": float(reward.numel()),
    }
...
def allowed_memory_type_ids(memory_signal_scope: str, rrca_signal_scope: str = "revision_only") -> set[int]:
    ...
    if scope == "explicit_negative":
        return {2, 3}
    if scope == "rrca":
        return {3} if str(rrca_signal_scope) == "revision_only" else {2, 3}
    return {2, 3}
...
def memory_signal(
    ...
    if args.memory_signal_scope == "rrca":
        ...
        if regret_type_id <= 0 or regret_type_id == 1 or phi_penalty <= 0.0:
            return None
...
def select_with_policy(
    ...
    item_features: np.ndarray | torch.Tensor | None,
    future_predictor: FuturePredictor | None,
    ...
    if bool(args.use_future_predictor) and float(args.predictor_beta) > 0.0:
        if future_predictor is None or item_features is None:
            raise RuntimeError("--use_future_predictor requires a loaded predictor and item feature table")
        action_items, sid_paths = decode_with_future_predictor(
            future_predictor,
            item_features,
            obs,
            decoder,
    ...
    value, v_seq = critic_q(critic, out, sid_paths)
    return out, action_items, sid_paths, logp, entropy, value, v_seq
...
def train_rollout_batch(
    ...
    for _step in range(int(args.max_steps)):
        ...
        _out, action_items, sid_paths, logp, entropy, value, v_seq = select_with_policy(
            actor, critic, state_obs, decoder, args, rng, memory, item_features, future_predictor
        )
        model_q = torch.zeros(batch_size, dtype=torch.float32, device=env.device)
        if model_bundle is not None and item_features is not None and float(args.model_q_weight) > 0.0:
            model_q = compute_model_q(model_bundle, item_features, state_obs, action_items, args)
        ...
        with torch.no_grad():
            if memory is not None:
                ...
                pool_tokens, pool_phis = None, None
                next_out = actor(actor_obs(env.current_observation))
            if str(args.critic_type) == "action_q":
                _next_items, next_sid_paths = decoder.decode_logits(
                    next_out["sid_logits"],
                    fallback_items=fallback_items_from_history(env.current_observation),
                    top_k=args.decode_top_k,
                    device=env.device,
                    mode="argmax",
                    rng=rng,
                ...
                next_value = target_critic({"context_list": next_out["context_list"], "target_sid": next_sid_paths})["q"]
            else:
    ...
    base_target_values: list[torch.Tensor] = []
    model_q_values: list[torch.Tensor] = []
    model_q_aux_values: list[torch.Tensor] = []
    ...
    for rec in records:
        ...
        base_target = step_reward + float(args.gamma) * (1.0 - rec["done"]) * rec["next_value"]
        model_q_aux = torch.zeros_like(base_target)
        if model_bundle is not None and float(args.model_q_weight) > 0.0:
            model_q_aux = float(args.model_q_weight) * rec["model_q"].detach()
        target = base_target + model_q_aux
        ...
        base_target_values.append(base_target[mask].detach())
        target_values.append(target[mask].detach())
        model_q_values.append(rec["model_q"][mask].detach())
        model_q_aux_values.append(model_q_aux[mask].detach())
    ...
    _, sid_vocab_size = infer_sid_spec(dense_item2sid)
    offline_aux_loss, offline_aux_metrics = compute_offline_aux_loss(
        actor,
        batch,
        dense_item2sid,
        sid_vocab_size,
    ...
    total_loss = (
        ...
        + float(args.offline_aux_weight) * offline_aux_loss
    )
    ...
    base_target_cat = torch.cat(base_target_values) if base_target_values else torch.zeros(1, device=env.device)
    model_q_cat = torch.cat(model_q_values) if model_q_values else torch.zeros(1, device=env.device)
    model_q_aux_cat = torch.cat(model_q_aux_values) if model_q_aux_values else torch.zeros(1, device=env.device)
    metrics = {
        "n": n_active,
        "loss": float(total_loss.detach().cpu()),
        "critic_type_action_q": 1.0 if str(args.critic_type) == "action_q" else 0.0,
        ...
        "offline_aux_weight": float(args.offline_aux_weight),
        **offline_aux_metrics,
        ...
        "td_base_target_mean": float(base_target_cat.mean().detach().cpu()),
        "model_q_mean": float(model_q_cat.mean().detach().cpu()),
        "model_q_aux_mean": float(model_q_aux_cat.mean().detach().cpu()),
        "model_q_weight": float(args.model_q_weight) if model_bundle is not None else 0.0,
    ...
    if hasattr(critic, "token_weight"):
...
def main() -> None:
    ...
    sid_levels, sid_vocab_size = infer_sid_spec(dense_item2sid)
    item_features_np = np.load(args.item_features_npy, mmap_mode="r")
    feature_shape = item_features_np.shape
    ...
    use_future_predictor = bool(args.use_future_predictor) and float(args.predictor_beta) > 0.0
    if use_future_predictor:
        future_predictor = load_future_predictor(args.predictor_checkpoint, int(feature_shape[1]), device)
        print(
            "[predictor] enabled "
            f"beta={args.predictor_beta:.4f} regret_eta={args.predictor_regret_eta:.4f} "
            f"candidate_k={args.predictor_candidate_k} score={args.predictor_score_mode} norm={args.predictor_score_norm}"
        )
    elif args.use_future_predictor:
        print("[predictor] disabled because predictor_beta <= 0; using original decoder")
    model_bundle = None
    use_model_based_predictor = bool(args.use_model_based_predictor) and float(args.model_q_weight) > 0.0
    if use_model_based_predictor:
        model_bundle = load_model_based_bundle(
            args.future_value_checkpoint,
            args.model_predictor_checkpoint,
            int(feature_shape[1]),
            device,
        )
        print(
            "[model_predictor] enabled "
            f"weight={args.model_q_weight:.4f} gamma={args.model_q_gamma:.4f} "
            f"clip={args.model_q_clip:.4f} mode={args.model_q_mode}"
        )
    elif args.use_model_based_predictor:
        print("[model_predictor] disabled because model_q_weight <= 0; using simulator-only critic target")
    item_features = maybe_cache_item_features_on_gpu(
        item_features_np,
        device,
        (use_future_predictor or use_model_based_predictor) and args.cache_item_features_on_gpu,
    )
    if str(args.critic_type) == "action_q":
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        for batch in pbar:
            metrics = train_rollout_batch(
                ...
                item_features, future_predictor, model_bundle,
            )
            ...
            pbar.set_postfix(
                ...
                off=view.get("offline_aux_loss", 0.0),
            )
        pbar.close()
        ...
        print(
            ...
            f"offAux={epoch_metrics.get('offline_aux_loss', float('nan')):.4f} "
            f"offTok={epoch_metrics.get('offline_aux_token_acc', float('nan')):.3f} "
            f"offPos={epoch_metrics.get('offline_aux_pos_share', float('nan')):.3f} "
            f"V={epoch_metrics.get('v_phi_mean', float('nan')):.4f} "
            f"Qhat={epoch_metrics.get('q_hat_mean', float('nan')):.4f} "
            f"modelQ={epoch_metrics.get('model_q_mean', float('nan')):.4f} "
            f"modelAux={epoch_metrics.get('model_q_aux_mean', float('nan')):.4f} "
