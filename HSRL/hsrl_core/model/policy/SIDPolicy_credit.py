...
class SIDPolicy_credit(nn.Module):
    ...
    def get_sara_logits(self, feed_dict, pool_tokens=None, pool_phis=None):
        """Generate SID logits with optional regret-memory soft intervention.

        pool_tokens accepts either [M,L] for a shared pool or [B,M,L] for
        per-active-trajectory regret memories. pool_phis follows [M] / [B,M].
        """
        enc = self.encode_state(feed_dict)
        context = enc["state_emb"]
        ...
        if pool_tokens is not None and pool_phis is not None and pool_tokens.numel() > 0:
            ...
            if pool_tokens.dim() != 3 or pool_phis.dim() != 2:
                raise ValueError(
                    "pool_tokens must be [M,L] or [B,M,L], and pool_phis must be [M] or [B,M]."
                )
            if pool_tokens.shape[0] != batch_size or pool_phis.shape[0] != batch_size:
                raise ValueError(
                    f"Batched regret pool size mismatch: batch={batch_size}, "
                    f"tokens={tuple(pool_tokens.shape)}, phis={tuple(pool_phis.shape)}"
                )
