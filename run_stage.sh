...
run_01_codebook() {
  ...
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/01_build_codebook.py \
...
run_02_item_sid() {
  ...
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/02_build_item_sid.py \
    --embeddings_parquet "${EMBEDDINGS_PARQUET}" \
...
run_03_split() {
  ...
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/03_split_data.py \
...
run_04_hpn() {
  ...
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/04_train_hpn_warmstart.py \
...
run_05_user_response() {
  ...
  echo "[output] ${URM_EFFECTIVE_TRAIN_FILE}"
  echo "[output] ${URM_EFFECTIVE_VAL_FILE}"
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/05_train_user_response.py \
    --train_file "${TRAIN_TSV}" \
    --val_file "${VAL_TSV}" \
    --test_file "${TEST_TSV}" \
    --embeddings_parquet "${EMBEDDINGS_PARQUET}" \
...
run_06_sid_rl() {
  ...
  need_file "${DENSE_ITEM2SID_NPY}" "请先运行 ./run_stage.sh 02"
  if [[ "${SID_TRAIN_MODE}" == "online_env" ]]; then
    need_file "${URM_LOG_PATH}" "请先运行 ./run_stage.sh 05"
    need_file "${URM_FEATURE_CACHE_NPZ}" "请先运行 ./run_stage.sh 05"
    need_file "${URM_EFFECTIVE_TRAIN_FILE}" "请先运行 ./run_stage.sh 05"
    need_file "${URM_EFFECTIVE_VAL_FILE}" "请先运行 ./run_stage.sh 05"
    need_file "${HPN_CHECKPOINT}" "请先运行 ./run_stage.sh 04"
  elif [[ "${SID_TRAIN_MODE}" == "offline_transition" ]]; then
    need_dir "${TRANSITION_ROOT}/train" "请先运行 Regret/scripts/02_split_transitions.py 生成 transition parquet"
    need_dir "${TRANSITION_ROOT}/val" "请先运行 Regret/scripts/02_split_transitions.py 生成 transition parquet"
    need_file "${DENSE_ITEM_FEATURES_NPY}" "请先准备 dense_item_features.npy"
  else
    echo "[error] unknown SID_TRAIN_MODE=${SID_TRAIN_MODE}" >&2
    exit 1
  fi
  echo "[mode] ${SID_TRAIN_MODE}"
  echo "[output] ${SID_SAVE_PATH}_actor"
  echo "[output] ${SID_SAVE_PATH}_critic"
  SHUFFLE_TRAIN_FILES_ARG=""
  if [[ "${SID_SHUFFLE_TRAIN_FILES}" == "1" ]]; then
    SHUFFLE_TRAIN_FILES_ARG="--shuffle_train_files"
  fi
  SAMPLE_ACROSS_FILES_ARG="--no-train_sample_across_files"
  if [[ "${SID_TRAIN_SAMPLE_ACROSS_FILES}" == "1" ]]; then
    SAMPLE_ACROSS_FILES_ARG="--train_sample_across_files"
  fi
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/06_train_yambda_sid.py \
    --train_mode "${SID_TRAIN_MODE}" \
    --device "${DEVICE}" \
    ...
    --max_seq_len "${SID_MAX_SEQ_LEN}" \
    --max_train_rows "${SID_MAX_TRAIN_ROWS}" \
    --max_val_rows "${SID_MAX_VAL_ROWS}" \
    --read_batch_size "${SID_READ_BATCH_SIZE}" \
    --offline_epochs "${SID_OFFLINE_EPOCHS}" \
    --shuffle_buffer_size "${SID_SHUFFLE_BUFFER_SIZE}" \
    ${SHUFFLE_TRAIN_FILES_ARG} \
    ${SAMPLE_ACROSS_FILES_ARG} \
    --offline_reward_threshold "${SID_OFFLINE_REWARD_THRESHOLD}" \
    --offline_reward_temperature "${SID_OFFLINE_REWARD_TEMPERATURE}" \
    --offline_bc_weight "${SID_OFFLINE_BC_WEIGHT}" \
    --offline_avoid_weight "${SID_OFFLINE_AVOID_WEIGHT}" \
    --offline_critic_weight "${SID_OFFLINE_CRITIC_WEIGHT}" \
    --offline_entropy_weight "${SID_OFFLINE_ENTROPY_WEIGHT}" \
    --offline_grad_clip "${SID_OFFLINE_GRAD_CLIP}" \
...
run_07_eval() {
  ...
  MPLCONFIGDIR="${MPLCONFIGDIR}" "${PYTHON_BIN}" -u HSRL/07_eval_candidate_ranking.py \
    --eval_file "${TEST_TSV}" \
    --embeddings_parquet "${EMBEDDINGS_PARQUET}" \
    --orig2dense_npy "${ORIG2DENSE_NPY}" \
    --dense_item2sid_npy "${DENSE_ITEM2SID_NPY}" \
