...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--regret_memory_delta_mode",
        choices=["step", "time_tick", "time_hour"],
        default="step",
        help=(
            "Distance used in Phi=gamma^Delta*psi for precomputed B_rev. "
            "step uses user_step_idx distance; time_tick uses raw timestamp ticks; "
            "time_hour uses elapsed hours from raw timestamps and is the paper-aligned mainline."
        ),
    )
    parser.add_argument(
        "--regret_memory_scope",
        choices=["all_failed", "explicit_negative", "revision_only", "none"],
        default="explicit_negative",
        help=(
            "Scope for precomputed B_rev snapshots. all_failed stores low_play/dislike/unlike; "
            "explicit_negative stores dislike/unlike only; revision_only stores unlike only."
        ),
    )
...
def step_failed_memory_signal(step: dict, scope: str) -> tuple[int, float] | None:
    ...
    if scope == "none":
        return None
    if regret_type == "unlike" and scope in {"all_failed", "explicit_negative", "revision_only"}:
        return 3, max(strength, 1e-6)
    if regret_type == "dislike" and scope in {"all_failed", "explicit_negative"}:
        return 2, max(strength, 1e-6)
    if regret_type == "low_play" and scope == "all_failed":
...
def attach_regret_memory_snapshots(
    ...
    timestamp_unit_seconds = float(timestamp_unit_seconds)

    def delta_tr(current_step: dict, entry: dict[str, Any]) -> float:
        if delta_mode == "step":
            return float(max(0, int(current_step["user_step_idx"]) - int(entry["user_step_idx"])))
        current_time = int(current_step["step_end_time"])
        entry_time = int(entry["step_time"])
        delta_ticks = float(max(0, current_time - entry_time))
        if delta_mode == "time_tick":
            return delta_ticks
        if delta_mode == "time_hour":
            return delta_ticks * timestamp_unit_seconds / 3600.0
        raise ValueError(f"Unsupported regret_memory_delta_mode: {delta_mode}")
    ...
    for step_pos, step in enumerate(steps):
        ...
        if memory_size > 0:
            for entry in memory_entries:
                delta_value = delta_tr(step, entry)
                phi = float(entry["strength"]) * (gamma ** delta_value)
                if phi < min_phi:
                    continue
        ...
        if args.trajectory_mode == "session_run":
            ...
            if args.precompute_regret_memory:
                memory_stats = attach_regret_memory_snapshots(
                    ...
                    delta_mode=str(args.regret_memory_delta_mode),
                    timestamp_unit_seconds=float(args.timestamp_unit_seconds),
                )
