...
def summarize_events(events: list[dict], weights: RewardWeights) -> dict:
    ...
    paper_base_reward = (
        float(max_play_ratio)
        + float(feedback["effective_like"])
        - float(feedback["effective_dislike"])
    )
    paper_revision_psi = (
        -float(feedback["effective_unlike"])
        + float(feedback["effective_undislike"])
    )
    # For the current session-run tuple, Delta t = 0, so gamma^Delta t = 1.
    paper_effective_reward = paper_base_reward + paper_revision_psi
    return {
        ...
        **feedback,
        ...
        "paper_base_reward": float(paper_base_reward),
        "paper_revision_psi": float(paper_revision_psi),
        "paper_effective_reward": float(paper_effective_reward),
